...
class Themevast_Megamenu_Block_Megamenu extends Mage_Catalog_Block_Navigation
{
    ...
    public function drawMegamenuMain()
    {
        ...
        if(count($cats)){
            $item = 1;
            if(Mage::getStoreConfig('megamenu/general/vmenutype') == 1){
                 foreach ($cats as $cat){
    ...
    public function drawMegamenuItem($category, $level = 0, $last = false, $item)
    {
        ...
        $html .= '<div class="level-top">';
        $caticon?$html .= '<img class="level-top-icon" src='.Mage::getBaseUrl('media').'catalog/category/'.$caticon.'>':'';
        ...
        $isOutermost = false, $outermostItemClass = '', $childrenWrapClass = '', $noEventAttributes = false)
    {
       ...
       /* if ($hasActiveChildren && !$noEventAttributes) {
        ...
        $html[] = '<span>' . $this->escapeHtml($category->getName()) . '</span>';
        $html[] = '</a>';
        if(Mage::getStoreConfig('megamenu/general/vmenutype') == 2 && $hasActiveChildren){ $html[] = '<span class="toggle_cat"></span>';};
        // render children
        $htmlChildren = '';
        $j = 0;
        foreach ($activeChildren as $child) {
    ...
    public function drawCategoriesGrid()
    {
        $html ='';
        $cats = Mage::helper('catalog/category')->getStoreCategories();
        if(Mage::getStoreConfig('megamenu/general/cat_grid_ids') == ''){
            if(count($cats)){
                $item = 1;
                     foreach ($cats as $cat){
                    $html .= $this->drawCategoryItem($cat,0,false,$item);
                    $item++;
                    }
            }
        }else{
            $cat_ids = explode(',', Mage::getStoreConfig('megamenu/general/cat_grid_ids'));
            if(count($cats)){
                $item = 1;
                    foreach ($cats as $cat){
     ...
     public function drawCategoryItem($category, $level = 0, $last = false, $item)
    {
        $html = '';
        if (!$category->getIsActive()) return $html;
        $id = $category->getId(); 
        $columns = round(12/Mage::getStoreConfig('megamenu/general/cat_grid_columns'));
        $html .= '<div class = "col-md-'.$columns.' col-sm-6 col-sms-6 col-smb-12 container clearfix">';
        $_imgUrl = Mage::getModel("catalog/category")->load($category->getId())->getThumbnail(); 
        // --- Sub Categories ---
        $activeChildren = $this->getActiveChildren($category, $level);
        $_imgHtml = '<p class="category-image"><a href="'.$this->getCategoryUrl($category).'"><img src="'.Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA).'catalog/category/'.$_imgUrl.'" alt="'.$this->escapeHtml($category->getName()).'" title="'.$this->escapeHtml($category->getName()).'" /></a></p>';
        ...
        $html .= '<div id="megamenu_catid_' . $id . '" class="megamenu nav-' .$item. ' megamenu_no_child">';

        // --- Top Menu Item ---
        $name = $this->escapeHtml($category->getName());
        $name = str_replace('&nbsp;', '', $name);
        $html .= '<div class="level-top">';
        $html .= '<a href="'.$this->getCategoryUrl($category).'"><span>' . $name . '</span></a>';
        $html .= '</div>';       
        // --- Add Dropdown block (hidden) ---

            // --- Dropdown function for hide ---
            $html .= '<ul id="sub_cat' . $id . '" class="sub_cat">';
                // --- draw Sub Categories ---
                if (count($activeChildren)){
                ...
                $name = str_replace(' ', '&nbsp;', $name);
                    ...
                    $html .= '<li><a class="itemMenuName level' . $level . $active . $ClassNoChildren . '" href="' . $this->getCategoryUrl($child) . '"><span>' . $name . '</span></a></li>';
